...
from cdqa.reader.utils_squad import (
    read_squad_examples,
...
def train(args, train_dataset, model, tokenizer, verbose_logging=False):
    """ Train the model """
...
def evaluate(input_file, args, model, tokenizer, prefix="", verbose_logging=False):
...
def load_and_cache_examples(
    ...
    cached_features_file = os.path.join(
        os.path.dirname(input_file) if isinstance(input_file, str) else "",
        "cached_{}_{}_{}".format(
...
def predict(input_file, args, model, tokenizer, prefix="", verbose_logging=False):
    dataset, examples, features = load_and_cache_examples(
        input_file,
        args,
        tokenizer,
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {
                ...
                "token_type_ids": None
                if args.model_type == "xlm"
                else batch[1],  # XLM don't use segment_ids
    ...
    if args.model_type in ["xlnet", "xlm"]:
        # XLNet uses a more complex post-processing procedure
        out_eval, final_prediction = write_predictions_extended(
            examples,
            features,
            all_results,
            args.n_best_size,
...
class Reader(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if self.local_rank == -1 or torch.distributed.get_rank() == 0:
            ...
            torch.save(
                self.get_params(), os.path.join(self.output_dir, "training_args.bin")
            )
        ...
        if self.local_rank in [-1, 0]:
            ...
            if self.eval_all_checkpoints:
                checkpoints = list(
                    ...
                    for c in sorted(
                        glob.glob(
                            self.output_dir + "/**/" + WEIGHTS_NAME, recursive=True
                        )
                    )
                )
                if self.verbose_logging:
    ...
    def predict(self, X):
        ...
        out_eval, final_prediction = predict(
            input_file=X,
            args=self,
            model=self.model,
            tokenizer=self.tokenizer,
